...
- [oh-my-claudecode](https://github.com/nicholasgriffintn/oh-my-claudecode) — **선택 사항 (권장)**

### OMC 유무 비교

| | OMC 있음 | OMC 없음 |
|---|---------|---------|
| **에이전트** | `document-specialist` (리서치 최적화 프롬프트) | `general-purpose` (자동 폴백) |
| **웹 검색** | 가능 | 가능 |
| **리서치 품질** | 높음 — 랜드스케이프 스캔에 특화된 프롬프트 | 양호 — 동일한 웹 검색 도구, 전문 프레이밍 없음 |
| **속도** | ~2-3분 | ~2-3분 |

OMC 없이도 Claude Code 내장 `general-purpose` 에이전트로 폴백되어 동작합니다. 더 높은 리서치 품질을 원하면 OMC를 설치하여 `document-specialist` 에이전트의 최적화된 시스템 프롬프트를 활용하세요.
